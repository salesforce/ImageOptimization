...
check_maven_version() {
    local mvn_ver=$(mvn -v | head -n1 | awk '{print $3}')
    if [[ "${mvn_ver%%.*}" -lt 3 ]] || { [[ "${mvn_ver%%.*}" -eq 3 ]] && [[ "${mvn_ver#*.}" < 3 ]]; }; then
...
install_system_packages() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        brew install "${MACOS_PKGS[@]}" 2>&1 || log_info "Brew reported issues, attempting to proceed..."
        hash -r
...
install_app() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        curl -sL --max-time 60 "$png_url" -o "$TEMP_DIR/pngout.tar.gz"
        tar -xzf "$TEMP_DIR/pngout.tar.gz" -C "$TEMP_DIR"
        local arch="x86"; [[ $(uname -m) == "x86_64" ]] && arch="x64"
        find "$TEMP_DIR" -path "*/$arch/pngout" -exec cp {} "$bin_dir/" \;
        chmod +x "$bin_dir/pngout"
    fi

    log_info "Compiling JFIFREMOVE..."
    curl -sL --max-time 30 "https://raw.githubusercontent.com/x2q/imgopt/master/jfifremove.c" -o "$TEMP_DIR/jfifremove.c"
    gcc -O2 -o "$bin_dir/jfifremove" "$TEMP_DIR/jfifremove.c"
...
if [ "${1:-}" == "uninstall" ]; then uninstall_app; else install_app; fi